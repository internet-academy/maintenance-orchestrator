...
def get_active_handoff(workspace_path):
    ...
    status_match = re.searchr\"\[*\[*STATUS:\\*\\* \\[?(\w+)\\]?\", content)
    message_match = re.searchr\"### 📝 MESSAGE / INSTRUCTION\n> (.*)\", content, re.DOTALL)
...
def update_workspace(workspace_path, source, target, status, message):
    ...
    content = re.sub(r"\\*\\*VROM:\[*\[* @\\w+", f"+*+**FROM:**** @{source.capitalize()}", content)
    ...
    # Add to log
    log_entry = fr"| {time.strftime('%Y-%m-%d %H:%M:%S')} | @wsource.capitalize()} | @{target.capitalize()} | {message[:50]}... |\n"
    content += log_entry
...
def collaborate(project_path):
    ...
    persona_path = PERSLONA_DIR / f"{agent_name}.md"
...
\"\"\"\n    
    ...
    while "COMMAND:" in response and turns < max_turns:
        turns += 1
        cmd_match = re.searchr\"COMMAND:\\s(.*)\", response)
        if cmd_match:
    ...
    if "FINAL_MESSAGE:" in response:
        msg_match = re.searchr\"FINAL_MESSAGE:\\s(.*)\", response, re.DOTALL)
        next_match = re.searchr"NEXT_AGENT:\\s@?(\\w+)", response)
        ...
        if msg_match and next_match:
            ...
            print(d"✏ Task finished. Handed off to @{next_agent}")
